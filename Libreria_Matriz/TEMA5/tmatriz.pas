...
{ Matriz }
 ...
 Function Moda_TSI_TID():integer;
  ...
  //Modelo de examen
  procedure cargar_Ram(f,c:Integer);
  procedure Trans_TSD();
  procedure Trans_TII();
  procedure Transponer_DP();
  procedure Ord_TSD();
  procedure ord_TSI();
  procedure ord_TID();
  procedure Ord_TII();
  procedure Ord_Triangulares_DS();
  Procedure Ord_Triangulares_DP();
  procedure ord_MSelec();
  procedure Ord_Mfila();   //ordena fila por fila
  procedure Ord_1er_elem(); //1er elemento de cada fila
  end;
...
begin
 ...
 Tra();
  ...
  procedure Matriz.cargar_Ram(f, c: Integer);
  var
  fl,cl:Integer;
  begin
   self.setfils(f);
   ...
   Randomize;
    for fl:=1 to f do
    for cl:=1 to c do begin
   x[fl,cl]:=random(15);
    end;
  end;

  procedure Matriz.Trans_TSD;
  var f1,c1:integer;
begin
 for f1:=1 to (fils div 2) do begin
    for c1:=f1+1  to (cols-1) do begin
        inter_Ele(f1,c1,fils-c1+1,cols-f1+1);
     end;
  end;
 end;

  procedure Matriz.Trans_TII;
...
begin
 ...
 for c1:=1  to (coLS DIV 2) do begin
  for f1:=c1+1 to (fils-c1) do begin
      i:=I+1;
     inter_Ele(f1,c1,fils+1-C1,FILS+1-F1);
     end;
  end;
end;

 procedure Matriz.Transponer_DP;
 begin
  self.Trans_TII();
  self.Trans_TSD();
  end;

 procedure Matriz.Ord_TSD;
 var f1,c1,i,f2,c2:integer;
begin
  ...
  procedure Matriz.ord_TID;
 var
   f1,c1,i,f2,c2:integer;
 begin
   for f1:=1 to fils-1 do
   begin
     for c1:=1 to cols-f1 do
     begin
       for f2:=f1+1 to fils do
       begin
         i:=1;
         for c2:=i to cols-f2 do
         begin
           if (x[f1,c1] > x[f2,c2]) then
             inter_Ele(f1,c1,f2,c2);
         end;
...
begin
 ...
 procedure Matriz.Ord_Triangulares_DS;
 begin
   self.ord_TSI();
   self.ord_TID();
 end;

 procedure Matriz.Ord_Triangulares_DP; //Ascendente
 begin
  self.Ord_TSD();
  self.Ord_TII();
 end;

 procedure Matriz.ord_MSelec;
   var
    ...
    for d:=p+1 to dim do
     begin
      ...
      cp:=((p-1) MOD cols)+1;
       ...
       if (x[fd,cd])< (x[fp,cp]) then
           inter_Ele(fd,cd,fp,cp);
       end;

 end;

 procedure Matriz.Ord_Mfila;
 var
  k,aux,f,c:integer;
 begin
   for f:=1 to fils do
    for c:=1 to cols-1 do begin
     for k:=c+1 to cols do
     begin
     if (x[f,c]>x[f,k])then begin
     self.Inter_ele(f,c,f,k); end;
     end;
   end;
 end;

 procedure Matriz.Ord_1er_elem;
 var
  f,c,k:integer;
 begin
  c:=1;
  for f:=1 to fils-1 do
   for k:=f+1 to fils do
   begin
    if x[f,c]>x[k,c] then
    self.Inter_ele(f,c,k,c);
   end;
 end;
