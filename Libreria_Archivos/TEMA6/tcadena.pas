   ...
   cadena=class
     private
       ...
       public
        ...
        function encriptar_pal(x1:string; d:byte):string;
        function desencriptar_pal(x1:string; d:byte):string;
...
function cadena.encriptar_pal(x1: string; d: byte): string;
...
function cadena.desencriptar_pal(x1: string; d: byte): string;
  const abc = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
var n,i : Cardinal;
      p : byte;
      r : string;
...
begin
  i := 1;
  n := '';
  while ((i<=self.dim)AND not(x[i] in Numeros)) do begin
    i:= i + 1;
  end;
  while ((i<=self.dim)AND (x[i] in Numeros)) do begin
    n := n+x[i];
    i:= i + 1;
  end;
  result := n;
...
begin
   begin
  pal:='';           //sep[ . | ; |  | , | : | - | _ ]
  while(pos<=dim)and(x[pos] in ['0'..'9','.'])do begin
    pal:=pal+x[pos];  //pal='ho'
    pos:=pos+1;          //pos=5
  end;
  while(pos<=dim)and(not (x[pos] in ['0'..'9','.']))do begin
    pos:=pos+1;
  end;
end;
end;
